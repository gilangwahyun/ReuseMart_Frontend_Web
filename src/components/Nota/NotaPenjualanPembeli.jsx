...
const NotaPengambilanDocument = ({ data }) => {
  const { invoiceNumber, jadwal, transaksi, pembeli, buyerEmail, alamat, items, qcOfficer } = data;
...
const NotaPenjualanPembeli = () => {
  ...
  const { invoiceNumber, jadwal, transaksi, pembeli, buyerEmail, alamat, items, qcOfficer } = notaData;
  const totalAmount = items.reduce((sum, item) => sum + (item.harga * (item.jumlah || 1)), 0);
  ...
  return (
    <Container className="my-4">
      <div className="d-flex justify-content-between mb-4">
        ...
        <div>
          <PDFDownloadLink
            document={<NotaPengambilanDocument data={notaData} />}
            fileName={`Nota_Pengambilan_${id_jadwal}.pdf`}
