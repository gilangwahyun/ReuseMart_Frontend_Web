...
const Navbar = ({ onKategoriSelect = () => {}, onSearch = () => {} }) => {
  ...
  useEffect(() => {
    ...
    if (userData) {
      try {
        ...
        console.log("User data loaded:", { id: user.id_user, role: role });
      } catch (error) {
  ...
  const handleProfileClick = () => {
    if (isLoggedIn && userId) {
      console.log("Profile click - User role:", userRole, "User ID:", userId);
      
      if (userRole === "penitip") {
        console.log("Navigating to penitip profile");
        navigate(`/DashboardProfilPenitip/${userId}`);
      } else {
        console.log("Navigating to pembeli profile");
